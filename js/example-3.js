...
(function () {
    ...
    const config = {
        fabBgColor: scriptTag.getAttribute("data-fab-bg-color") || "#333333",
        fabIcon: scriptTag.getAttribute("data-fab-icon"),
        fabCloseIcon: scriptTag.getAttribute("data-fab-close-icon"),
        iframeSrc: scriptTag.getAttribute("data-iframe-src") || "",
        iframeWidth: scriptTag.getAttribute("data-iframe-width") || 320,
    ...
    const widgetHTML = `
        <div class="techvill-widget-fab-container">
            <button id="techvill-widget-fab" class="techvill-widget-fab-button" style="background-color: ${config.fabBgColor}">
                <img id="techvill-widget-icon" src="${config.fabIcon}" alt="open" class="techvill-widget-fab-icon"/>
            </button>
        </div>
    ...
    const fabImg = document.getElementById("techvill-widget-icon");
    const chatModal = document.getElementById("techvill-widget-chat-modal");

    // Default SVG icon if no fabIcon is provided
    if (!config.fabIcon) {
        fabButton.innerHTML = `
    ...
    fabButton.addEventListener("click", function () {
        if (isOpen) {
            ...
            fabImg.src = config.fabCloseIcon || "";
            fabImg.alt = "Close";
            if (!config.fabCloseIcon) {
                fabButton.innerHTML = `
