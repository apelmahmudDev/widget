...
(async function () {
	...
	const config = {
		...
		fabIcon:
			scriptTag.getAttribute("data-fab-icon") ||
			"https://i.ibb.co/CthkwCV/img-robot-face.png",
	...
	const widgetHTML = `
        <div class="techvill-widget-fab-container">
            <button id="techvill-widget-fab" class="techvill-widget-fab-button" style="background-color: ${
							config.fabBgColor
						}; display: ${chatbot.status === "Active" ? "flex" : "none"};">
                <img src="${
									config.fabIcon
								}" alt="open" class="techvill-widget-fab-icon" />
            </button>
        </div>
